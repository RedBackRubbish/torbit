...
//
// ⚠️ INVARIANT: WebContainer + Next.js MUST use version 14.2.x
// Next.js 15+/16+ crash in WebContainer with various errors:
// - "Cannot access entryCSSFiles without a work store"
// - Turbopack WASM binding issues
...
`)
        ...
        // 3. Ensure globals.css exists (Tailwind v3 syntax)
...
`)
        ...
        // 5. Tailwind v3 config will be auto-generated by Next.js if missing
        // Only create one if the AI didn't generate one and we need customizations
        
        // 6. Ensure postcss.config.mjs exists (Tailwind v3 style)
...
`)
        ...
        try {
          ...
          if (pkgFile) {
            ...
            // CRITICAL: Ensure Next.js 14.2.28 is used
            // Next.js 15+/16+ crash in WebContainer with various errors
            // Next.js 14 uses webpack by default (no flags needed)
            ...
            // Ensure dev script exists (remove any Turbopack flags)
            if (pkg.scripts?.dev) {
              // Remove any --turbo or --turbopack flags (they don't work in Next.js 14)
              if (pkg.scripts.dev.includes('--turbo') || pkg.scripts.dev.includes('--webpack')) {
                pkg.scripts.dev = pkg.scripts.dev.replace(/--turbo(pack)?/g, '').replace(/--webpack/g, '').trim().replace(/\s+/g, ' ')
                needsUpdate = true
              }
            } else {
            ...
            // Add missing essential deps (Tailwind v3 - consistent with AI-generated configs)
