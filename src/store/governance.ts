...
export interface SignedAuditBundleSummary {
  id: string
  projectId: string
  createdAt: number
  action: string
...
export interface GovernanceState {
  ...
  /** Customer-visible approval workflow records */
  approvals: GovernanceApprovalRequest[]
  /** Signed audit bundle summaries for trust reporting */
  signedBundles: SignedAuditBundleSummary[]
}
...
export interface GovernanceActions {
  ...
  /** Create an explicit approval request users can review */
  ...
  /** Record a signed bundle summary generated by server signing */
  recordSignedBundle: (bundle: Omit<SignedAuditBundleSummary, 'id' | 'createdAt' | 'projectId'>) => string

  /** Read approvals scoped to the active project */
  getProjectApprovals: () => GovernanceApprovalRequest[]

  /** Read pending approvals scoped to the active project */
  getPendingApprovals: () => GovernanceApprovalRequest[]

  /** Read the latest signed bundle scoped to active project */
  getLatestSignedBundle: () => SignedAuditBundleSummary | null
}
...
export const useGovernanceStore = create<GovernanceState & GovernanceActions>()(
  persist(
    immer((set, get) => ({
      ...
      requestApproval: ({ action, summary, requestedBy }) => {
        const now = Date.now()
        ...
        set((state) => {
          state.approvals.unshift({
            ...
            action,
            summary,
            requestedBy: requestedBy || 'torbit',
            requestedAt: now,
            status: 'pending',
          })
          if (state.approvals.length > 200) {
      ...
      resolveApproval: ({ requestId, status, resolvedBy, decisionNote }) => {
        set((state) => {
          const approval = state.approvals.find((item) => item.id === requestId)
          if (!approval) return
      ...
      recordSignedBundle: (bundle) => {
        const now = Date.now()
        const id = `sig_${now}_${Math.random().toString(36).slice(2, 8)}`
        const pid = get().projectId
        set((state) => {
          state.signedBundles.unshift({
            id,
            projectId: pid,
            createdAt: now,
            ...bundle,
          })
          if (state.signedBundles.length > 200) {
            state.signedBundles = state.signedBundles.slice(0, 200)
          }
        })
        return id
      },
      ...
      getProjectApprovals: () => {
        const pid = get().projectId
        return get().approvals.filter((item) => item.projectId === pid)
      },

      getPendingApprovals: () => {
        const pid = get().projectId
        return get().approvals.filter((item) => item.projectId === pid && item.status === 'pending')
      },

      getLatestSignedBundle: () => {
        const pid = get().projectId
        return get().signedBundles.find((bundle) => bundle.projectId === pid) || null
      },
    })),
    {
      ...
      partialize: (state) => ({
        ...
        signedBundles: state.signedBundles.slice(0, 200),
      }),
    }
  )
)
...
export function useProjectApprovals() {
  return useGovernanceStore((s) => {
    const pid = s.projectId
    return s.approvals.filter((item) => item.projectId === pid)
  })
}
...
export function usePendingApprovals() {
  return useGovernanceStore((s) => {
    const pid = s.projectId
    return s.approvals.filter((item) => item.projectId === pid && item.status === 'pending')
  })
}

export function useLatestSignedBundle() {
  return useGovernanceStore((s) => {
    const pid = s.projectId
    return s.signedBundles.find((bundle) => bundle.projectId === pid) || null
  })
}
